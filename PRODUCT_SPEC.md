...
| NG3 | Public sign-up / SSO / OAuth — **ยกเว้น owner/admin invite QR แบบใช้ครั้งเดียว (DEC-081)** | requirement ระบุ username+password ที่แอดมินสร้างเท่านั้น (ออกแบบ `auth_provider` เผื่อ SSO ใน P2); ข้อยกเว้นเดียวคือบัญชีที่สร้างผ่าน invite token ที่ owner/admin ของ workspace หนึ่งออกให้โดยเจตนา ใช้ได้ครั้งเดียว หมดอายุใน 24 ชม. — ไม่มีหน้า/ปุ่ม "สมัครสมาชิก" สาธารณะที่เข้าถึงได้โดยไม่มี token ที่ถูกต้อง |
...
| metadata | jsonb NULL | link preview (P2); `forward: {sender_id, display_name, message_id, room_id, created_at}` (FR-MSG-011) — server-internal, serialized only as `forwarded_from` |
...
| `message.forward_max_messages` | 20 (1–100) | FR-MSG-011 |
| `message.forward_max_rooms` | 10 (1–50) | FR-MSG-011 |
...
#### FR-AUTH-008 สมัครสมาชิกผ่านคำเชิญ QR/ลิงก์ แบบใช้ครั้งเดียว — P1 · PH2 (DEC-081)
- **Behavior**: `GET /join/{token}` (public, ไม่ auth) ตรวจ token → คืนชื่อ workspace ให้แสดงหน้า preview ก่อนสมัคร ถูกเรียกซ้ำได้ไม่จำกัดครั้งโดยไม่ consume; `POST /join/{token} {username, password, display_name, locale?}` (public, ไม่ auth) → สร้าง user ใหม่ (username ต้องตรง `[a-z0-9._-]{3,32}` เหมือน §4.1) + เพิ่มเข้า workspace เป็น `role=member` แบบ atomic (`lockForUpdate` ภายใน transaction เดียว), ผูก token เป็น used ทันที, คืน `access_token`/`refresh_token` เหมือน login (ล็อกอินอัตโนมัติหลังสมัคร ไม่ต้อง login แยก)
- **AC**
  - [ ] token ไม่พบ/หมดอายุ/ถูกใช้ไปแล้ว/ถูก revoke → `404 INVITE_NOT_FOUND` / `410 INVITE_EXPIRED` / `409 INVITE_ALREADY_USED` / `410 INVITE_REVOKED` ตามลำดับ
  - [ ] username ไม่ตรง `[a-z0-9._-]{3,32}` → `422 VALIDATION_FAILED`; username ซ้ำ → `422 AUTH_USERNAME_TAKEN`; password ไม่ผ่าน policy → ใช้กฎเดียวกับ FR-AUTH-005 (`422 AUTH_PASSWORD_WEAK`)
  - [ ] สอง request พร้อมกันกับ token เดียวกัน (race) → ผ่านแค่ 1 คำขอ, อีกคำขอได้ `409 INVITE_ALREADY_USED` — พิสูจน์ด้วย HTTP request คู่ขนานจริง ไม่ใช่แค่ sequential
  - [ ] สมัครสำเร็จ → `must_change_password=false` (ผู้ใช้ตั้งรหัสเอง), `locale` ตามที่หน้า join ส่งมา (ไม่ hardcode), audit `auth.registered_via_invite` + `workspace.member_added`, broadcast `EVT-020 workspace.member_added` เหมือน FR-ADM-005
  - [ ] rate limit: `POST /join/{token}` = 5 req/นาที ต่อ ip; `GET /join/{token}` = 30 req/นาที ต่อ ip
- **Refs**: API-232, API-233, TC-AUTH-033..038
...
#### FR-WS-006 Owner/Admin สร้างคำเชิญ QR แบบใช้ครั้งเดียว — P1 · PH2 (DEC-081)
- **Behavior**: WS Admin/Owner กด "เชิญสมาชิกผ่าน QR" (บัญชี dropdown) → `POST /workspace-invites` (auth ws, ws admin+) → ออก token ใหม่ (64-hex random, เก็บเป็น sha256 hash เหมือน access/refresh token — ไม่เก็บ plaintext), `expires_at = now + 24h`, คืน `{id, token, join_url, expires_at}` ครั้งเดียว; client render QR จาก `join_url` ฝั่ง client เอง (ไม่ generate รูปที่ server); ปุ่ม "ยกเลิกลิงก์เชิญ" เรียก `DELETE /workspace-invites/{id}` (revoke, บล็อกการใช้ token ทันทีแม้ยังไม่หมดอายุ)
- **AC**
  - [ ] เฉพาะ WS Admin/Owner/System Admin เท่านั้นที่เรียกได้ (`403 WS_FORBIDDEN`)
  - [ ] token ที่ยังไม่ถูกใช้/revoke และไม่หมดอายุ ดูหน้า preview (`GET /join/{token}`) ซ้ำได้ไม่จำกัดครั้ง แต่สมัครสำเร็จได้แค่ 1 ครั้ง
  - [ ] revoke invite ของ workspace อื่น (แม้เป็น admin ของ ws ตน) → `404 INVITE_NOT_FOUND` (ไม่ leak)
  - [ ] rate limit ออก invite = 10 req/นาที ต่อ admin คนเดียว
  - [ ] ไม่มีหน้ารายการ invite ย้อนหลัง — client เก็บ invite ปัจจุบัน (ถ้ามี) ต่อ workspace ไว้ใน memory เท่านั้น หาย reload หน้า (token คืนมาครั้งเดียวจาก server จริง ๆ)
- **Refs**: API-230, API-231, TC-WS-022..024
...
- สร้าง schema ไว้ (Section 4) ; ไม่ทำใน v1 — **ยกเว้น** Forward (→ FR-MSG-011, DEC-083) และ Pin (→ FR-PIN-001, DEC-048) ที่ทำแล้ว; Reactions / Link preview ยังเป็น P2

#### FR-MSG-011 Forward messages to other rooms — P1 · PH4 (DEC-083)
- `POST /messages/forward {client_forward_id (uuid), source_room_id, message_ids[] (1..message.forward_max_messages), room_ids[] (1..message.forward_max_rooms)}` (API-047). Each target room receives a **new** message per source message, in source `seq` order, sent **by the forwarder**, carrying `forwarded_from = {sender_id, display_name, message_id, room_id, created_at}` of the ORIGINAL author (Telegram behaviour). Forwarding an already-forwarded message keeps its original `forwarded_from` (chain collapses to the first author). `display_name` is a snapshot taken at forward time.
- Body is copied verbatim; `reply_to` is dropped; the copy creates **no** `message_mentions` rows and never triggers the room AI bot. Every attachment is copied to **new storage objects** (original + every derived key) under a new attachment row owned by the forwarder — never shared — so deleting/expiring the source cannot remove the forwarded file and vice versa.
- **Guards** (all validated before anything is written): forwarder must be a current member of the source room (`403 ROOM_NOT_MEMBER`); every target must be a non-deleted, non-expired room in the same workspace where the forwarder is a current member (any failure → uniform `404`, like API-111); every source message must belong to `source_room_id`, be non-deleted and non-system, and every attachment must be `ready` (else `422 MSG_FORWARD_INVALID`); an **active secret source room is refused** (`422 MSG_FORWARD_INVALID`) — its content is promised to disappear at expiry (FR-ROOM-012); forwarding **into** a secret room is allowed (the copy expires with that room).
- **Integrity**: a forwarded message cannot be edited (`422 MSG_NOT_EDITABLE`) — editing under "Forwarded from X" would attribute fabricated text to X. Delete works as for any own message.
- **Idempotency / partial failure**: each copy's `client_message_id` is `uuid5(client_forward_id, "<source_message_id>:<target_room_id>")`; replaying the same request creates nothing new. Targets commit independently (one API-040 write transaction each, no cross-room lock); the response lists `failed_room_ids` for targets that failed after validation. Push/EVT behave exactly as a normal send (EVT-010, `NotifyMessage` per copy). Audit `message.forwarded` (actor, source room, target rooms, count).
- **AC**
  - [ ] copy appears in each target with `forwarded_from` of the original author, also in the EVT-010 payload (TC-MSG-061)
  - [ ] re-forward keeps the first author (TC-MSG-062)
  - [ ] attachment copy has new storage keys; deleting the source message leaves the forwarded file readable (TC-MSG-063)
  - [ ] secret source → 422; secret target → 201 (TC-MSG-064)
  - [ ] non-member source → 403; foreign/non-member/deleted target → 404, nothing written anywhere (TC-MSG-065)
  - [ ] deleted/system source message or not-ready attachment → 422 (TC-MSG-066)
  - [ ] limits enforced (TC-MSG-067); replay creates no duplicates (TC-MSG-068)
  - [ ] edit of a forwarded message → 422 MSG_NOT_EDITABLE (TC-MSG-069)
  - [ ] no mentions / no AI bot job from a forwarded `@user`/`@ai` body (TC-MSG-070)
  - [ ] raw `metadata` is never serialized; only `forwarded_from` (TC-MSG-071)
- **Client**: "Forward" in the message action menu (not on system/deleted/optimistic messages, not in an active secret room) → searchable multi-select room picker (🔒 marks secret targets) → confirm → toast; bubbles render "Forwarded from {name}" above the content on web and mobile (TC-WEB-078..080).
- **Refs**: API-047, EVT-010, TC-MSG-061..071, TC-WEB-078..080
...
- EVT-063 `notification.alert` on private-user.{uid}: envelope data `{id, room_id, kind}` (message/added_to_room/session_revoked), no message content; dispatch after commit. Browser title `(N) {ActiveWorkspaceName}` sums API-008 total_unread **across all workspaces** (unchanged by DEC-082 — the count is still the cross-workspace total, only the trailing label changed) while a workspace is active, e.g. `(7) TryCatch`; falls back to `(N) Banana Chat` with no active workspace, and to bare `Banana Chat` at zero/logout.
...
#### FR-ADM-015 System logo (branding.logo_path) — DEC-082
- System admin uploads/removes the installation's logo from the "Branding" section of the existing System settings page (FR-ADM-009's Filament page) — a dedicated `FileUpload` field, not one of the generic per-key settings fields, since a file path isn't a bool/array/string/numeric scalar that page's generic type-inference loop renders. Stored as `branding.logo_path` in the existing `app_settings` table (§4.2), on local disk (never the `public`/S3 disk — this installation's `/storage/` route proxies MinIO, and a system-wide logo has no meaningful workspace scope for the per-workspace `Attachment` pipeline to attach to).
- PNG/JPEG/WebP, max 2MB. Replacing a logo deletes the previous file; removing clears the setting and deletes the file. Both actions are audited via the existing `settings.updated` audit action (old/new `branding.logo_path`).
- Shown wherever the built-in Banana mark previously was: the rail/mobile-top-bar/login-page brand mark, and the public `/join/:token` invitation page's header mark — object-fit: contain, no crop, own square white tile (yellow tile is reserved for the built-in fallback). Falls back to the built-in mark on missing config OR image-load failure, without changing surrounding layout. No favicon change (out of scope for this pass) and no admin-configurable app *name* — only the mark.
- **AC**: `GET /api/v1/app-config` (public, pre-auth) returns `{logo_url}` — `null` when unset, else a URL cache-busted by the setting's own `updated_at` (`Cache-Control: immutable`), so already-open clients don't need to poll but a genuinely new upload always gets a fresh URL. `GET /api/v1/branding/logo` (public) streams the file or 404s. Uploading a new logo — or removing one — over an already-consumed FileUpload value on the same page load must not resurrect a deleted path on a later, unrelated Save (the field is explicitly cleared after both actions).
- **Refs**: TC-ADM-071a, TC-ADM-080..086

#### FR-ADM-016 Public Chat integration guide (admin menu) — documentation only, no DEC
- New Filament page `/admin/public-chat-integration` ("Public Chat guide", System group, beside Public Chat API Keys) — a concise, curated summary of the FR-PCHAT partner integration: choose-your-tier guidance, the before-you-start checklist, the create→deliver→chat→poll→close flow, one full worked request/response (API-200), the six-line HMAC canonical string, and the six most-hit error codes.
- **Single source of truth, not a second copy**: the exhaustive, code-verified reference (`apps/api/resources/docs/public-chat/README.md`, ~1100 lines — quickstart, all three tiers, every error code, rate limits, security notes, troubleshooting) and `apps/api/openapi.yaml` are offered as direct authenticated downloads (`Action::make('downloadGuide'|'downloadOpenApi')`, same admin auth as the page, no new route) rather than re-typed on the page, so there is exactly one place each fact can drift. The guide's tree **moved** from repo-root `docs/integration/public-chat/` to `apps/api/resources/docs/public-chat/` — the prod build context is `apps/api` (`infra/docker-compose.prod.yml`'s `x-api-image` + `Dockerfile.prod`'s `COPY . .`), and the repo-root `docs/` tree is outside it; the guide has to ship inside the built image to be downloadable in prod.
- The example request/response uses `config('app.url')`'s host, not a hardcoded domain, so the page reads correctly on both staging and prod.
- Written for two readers at once: a human admin, and an AI coding agent implementing a partner integration — hence clear semantic headings, language-tagged code fences, and the direct guide/OpenAPI downloads as the machine-readable source.
- **AC**: page renders for a system admin; both downloads stream the real files; the common-errors table's codes match `ApiException`'s actual public-chat codes (a renamed/removed code fails the test, not just looks wrong on the page).
- **Refs**: TC-ADM-087..090
...
| FR-AI-021 | Built-in virtual AI participant is present in every group (not DM), visible in header and @ai autocomplete; no manual bot invitation/removal. Only explicit standalone @ai tokens on newly created messages queue a reply; normal messages and message edits never trigger it. AI receives the addressed text as the only user turn, plus the room's recent messages as a background transcript in the system prompt (DEC-084), each prefixed with its sender's name and the bot's own earlier replies marked as its own, capped by ai.room_bot.history_messages and the provider input budget, with older lines the bot was already sent kept as a rolling room summary above them (DEC-085); attachments and private AI memories stay excluded. The room banner and the consent dialog state that room messages are sent. Existing provider/workspace/consent gate and daily quota apply; requesting member must remain active. Reply is attributed to a non-login bot account, quotes the source and is idempotent. Errors are visible as bot replies, never provider credentials/details. Provider must be configured; consent through existing AI Assistant. The reply streams into the posted message and tokens are estimated over the whole context. | TC-AI-100/101 |
...
| assign user เข้า ws | ❌ | ❌ | ❌ | ✅ |
| สร้างคำเชิญ QR ให้คนใหม่สมัครและ join เป็น member (DEC-081) | ❌ | ✅ | ✅ | ✅ |
...
AUTH_PASSWORD_REUSED, AUTH_CURRENT_PASSWORD_WRONG, AUTH_USERNAME_TAKEN,
INVITE_NOT_FOUND (404), INVITE_EXPIRED (410), INVITE_REVOKED (410), INVITE_ALREADY_USED (409),
...
MSG_REPLY_INVALID, MSG_TOO_MANY_ATTACHMENTS, MSG_FORWARD_INVALID (422 — FR-MSG-011),
...
| API-232 | GET | `/join/{token}` | public, rate-limited | — | `{workspace:{name}}` | AUTH-008 |
| API-233 | POST | `/join/{token}` | public, rate-limited | `{username, password, display_name, locale?, device?}` | เหมือน login (201) | AUTH-008 |
| API-234 | GET | `/app-config` | public, rate-limited | — | `{logo_url: string\|null}` | ADM-015 |
| API-235 | GET | `/branding/logo` | public, rate-limited | `?v=` | image bytes, 404 if unset | ADM-015 |
...
| API-230 | POST | `/workspace-invites` | ws admin | — | `{id, token, join_url, expires_at}` | WS-006 |
| API-231 | DELETE | `/workspace-invites/{id}` | ws admin | — | 204 | WS-006 |
...
| API-047 | POST | `/messages/forward` | member of source + every target | `{client_forward_id, source_room_id, message_ids[], room_ids[]}` | 201 (200 if pure replay) `{results:[{room_id, messages:[message]}], failed_room_ids:[]}` | MSG-011 |
...
| TC-MSG-061 | forward → copy in every target with original author's `forwarded_from`, also in EVT-010 payload | Feature |
| TC-MSG-062 | re-forward keeps the first author | Feature |
| TC-MSG-063 | forwarded attachment has new storage keys; deleting the source keeps the forwarded file | Feature |
| TC-MSG-064 | secret source → 422 MSG_FORWARD_INVALID; secret target → 201 | Feature |
| TC-MSG-065 | non-member source → 403; foreign/non-member/deleted target → 404 and nothing written | Feature |
| TC-MSG-066 | deleted/system message or not-ready attachment → 422 | Feature |
| TC-MSG-067 | message/room count limits → 422 | Feature |
| TC-MSG-068 | replaying the same client_forward_id creates no duplicates | Feature |
| TC-MSG-069 | editing a forwarded message → 422 MSG_NOT_EDITABLE | Feature |
| TC-MSG-070 | forwarded `@user`/`@ai` body creates no mentions and no bot job | Feature |
| TC-MSG-071 | raw metadata never serialized, only forwarded_from | Feature |
...
| FR-MSG-011 | 047 | 010 | MSG-061..071, WEB-078..080 |
...
| DEC-076 | **`preview_in_push` is read from the RECIPIENT, and FCM delivery moves to HTTP v1.** Two defects made the push path non-functional and one made it actively wrong. (a) `PushDecisionService::payload()` resolved `preview_in_push` from the `$sender` argument — the message author — so a user's own "do not show message text" setting governed what appeared on *other people's* lock screens and never their own; the internal path now takes the recipient explicitly (`payload(..., ?User $recipient)`), matching `NotifyPublicChatMessage.php` which always did this correctly. The parameter is nullable to keep the existing signature working, and its fallback is "show the preview" rather than "read the sender", so a caller that forgets to pass a recipient degrades to the safe default instead of silently restoring the inversion. (b) `FcmPushSender` posted to the legacy `https://fcm.googleapis.com/fcm/send` with an `Authorization: key=` header while its own docblock claimed to be a v1 sender; that API is decommissioned, so every send failed — and because an unset key short-circuits to a logged no-op, it failed silently. It now targets `/v1/projects/{id}/messages:send` with an OAuth2 bearer minted from a service-account assertion (RS256 via `firebase/php-jwt`, already a dependency, so `google/auth` is not introduced) and cached for just under its hour. (c) The credential was named `FCM_SERVER_KEY` and appeared in no `.env.example`, while Appendix A has always named `FCM_PROJECT_ID` and `FCM_CREDENTIALS_JSON`; config and the example now match the spec. The §10 `android`/`apns` blocks, which the legacy sender never emitted, are now sent, and stub mode logs at **warning** rather than info. | Not new behaviour — the spec described v1 all along (Appendix A credentials, a v1-shaped §10 payload), so this is compliance rather than scope, and no FR text changes. A DEC is filed because the *observable* payload changes: `data` values are now all strings (v1 rejects a non-string anywhere in the map), a dead token is a real 404 carrying `error.details[].errorCode` instead of a 200 with `results[].error`, and each send costs a second HTTP call when the cached OAuth token has expired. Rejected: adding `google/auth`, which pulls a large transitive tree to sign one JWT the project can already sign. Rejected: keeping the sender's setting as a "do not leak my text to others" feature — it is not what the field is named, not what the settings UI implies, and not what the public-chat path does. `TC-NOTI-014` was asserting the inverted behaviour and is corrected; a new regression test pins that a sender opting out cannot suppress a recipient's preview. | 2026-09-18 |
| DEC-077 | **Web Push ships as an installable PWA with an FCM Web service worker, dormant until an operator supplies Firebase credentials.** FR-NOTI-003 has always specified "Service worker + FCM web"; this builds it. Four decisions are worth recording. (a) **FCM Web rather than a direct RFC-8030/VAPID sender**, because `devices.push_token` already stores a single opaque registration token and the server sender is FCM HTTP v1 (DEC-076) — a direct sender would need `endpoint`/`p256dh`/`auth` columns and a second delivery path for one client platform. (b) **The service worker takes its Firebase config from its own registration URL's query string**, not from a build-time template. A worker is a static file at the origin root and cannot read `import.meta.env`; templating it would make `public/firebase-messaging-sw.js` a build artifact. The VAPID key is deliberately excluded from that URL — only the page needs it, for `getToken()`. (c) **No `onMessage` handler.** FCM delivers a push to the PAGE when a window client is visible and to the WORKER otherwise; the visible-tab case is already covered by the EVT-063 websocket alert, so registering `onMessage` would render a second popup for the same message — and the two paths carry different ids, so the dedupe gate could not suppress it. Websocket owns foreground, worker owns background. (d) **Push-only worker: no `fetch` handler, no precache.** A caching worker would have to exclude `/api`, `/broadcasting` and the Reverb socket, and §5.12 asks for no web offline shell. Also lands API-074 focus reporting from web, which had never been sent — the server's "silence a push for the room this device is reading" rule (FR-NOTI-002) was therefore dead for web clients. | **The iOS constraint is a platform rule, not a defect, and is surfaced in the UI rather than hidden**: web push on iPhone needs iOS 16.4+ **and** the site added to the Home Screen; in an ordinary Safari tab `Notification` does not exist at all. `webPushStatus()` returns a distinct `needs-install` for exactly that case so the panel can give the one instruction that helps, instead of "your browser is unsupported", which would be both wrong and a dead end. Everything is inert without `VITE_FIREBASE_*`: `isWebPushConfigured()` gates every entry point, the Firebase SDK is a lazy `import()` so it stays out of the initial bundle, and the worker registers but initialises nothing. Rejected: `vite-plugin-pwa`, which owns the worker lifecycle and pulls in Workbox precaching we explicitly do not want. Rejected: deriving the device id from the session — a device row is per browser profile and must outlive logins, so it is a `localStorage` id that degrades to a per-page id when storage throws (Safari private mode) rather than refusing to register. | 2026-09-18 |
| DEC-078 | **Calm Banana web chat (PLAN_ASTRA)**: scoped chat styles; full-width mobile conversation with Back; searchable two-line room rows and All/Unread; neutral timeline/pale-yellow outgoing bubbles; one labelled modal action menu (bottom sheet on mobile), delete confirmation; per-user/workspace/room message+pixel anchors, single-flight history, session-local new-arrival count, explicit latest/return controls. Read-receipt visibility/focus/anchor-mode gates remain authoritative. No API/schema change. | Fix visual clutter and invisible toolbar layout; protect reading position from incoming messages and changing content heights. Native app, authoritative first-unread landing, and customer/device acceptance remain separately tracked in PLAN_ASTRA. | 2026-09-19 |
| DEC-079 | **AI Assistant (`/ai*`) joins the DEC-078 chat shell — one sidebar, not two.** AppShell renders `AiConversationList` (styled like RoomList: `bc-conversations`/`bc-room-row`, real `NavLink`s) in place of `RoomList` when the route is `/ai*`; the pane (`AiChatPane`) reuses the shell's `bc-message`/`bc-message-bubble`/`bc-composer` classes instead of a separate Tailwind-only layout. Search (FR-AI-020) and all send/regenerate/edit-resend/version-switch/share behaviour (FR-AI-003/004/009/015/018) are unchanged. FR-AI-007's memories toggle becomes a real route, `/ai/memories`, deep-linkable and riding the same mobile is-conversation/is-chat-list machinery as `/ai/:id`, instead of client-only Zustand state. No API/schema change. Public Chat's staff queue (`/public-chat`) is explicitly out of scope: it is a full-width filterable table by its own design, not a sidebar+list surface, and restyling it is a separate slice. | Customer asked for the redesigned chat UI applied everywhere a chat list appears. The memories toggle was changed to a route because, as client-only state, tapping it at `/ai` on a phone left the panel unreachable: `.bc-chat-shell.is-chat-list .bc-main{display:none}` hides the outlet there and toggling a flag changes no AppShell class, so nothing else showed the panel — the same failure mode independently found and fixed for `AiConsentDialog` (now portaled to `document.body`, matching `MessageItem`'s existing dialog pattern) earlier in the same pass. A route sidesteps the whole class by construction rather than adding a third coupled flag to AppShell's `conversationOpen`/`listOpen`/`sidebarOpen`. | 2026-09-19 |
| DEC-081 | **Amend NG3 for one narrow path only: an owner/admin-issued, single-use, 24-hour-expiring invite token that lets exactly one new person create an account and join that specific workspace as `role=member`.** There is still no general public sign-up page, no SSO/OAuth, and no way to create an account without a valid, unexpired, unused, workspace-scoped invite token minted by that workspace's owner or admin (§6.1). The token is consumed atomically (`lockForUpdate()` inside one transaction, re-checked against `used_at`/`revoked_at`/`expires_at` after the lock — proven race-safe with genuinely concurrent HTTP requests, not just sequential test calls) so a race can never create two accounts from one token; the issuing owner/admin can revoke an unused token before it's scanned. The invite token itself is hashed (sha256, like access/refresh tokens) rather than stored plaintext (unlike `public_chat_rooms.code`, DEC-063) — this credential creates a real account, so it gets the stronger treatment. | Requested feature: owner/admin needs to onboard new people without going through the System-Admin-only Filament panel (FR-ADM-002/FR-ADM-005), which today is the only way an account or workspace membership can be created (§6.1: `assign user เข้า ws` = Ⓢ only). The bound is stated explicitly so this cannot be read as license for open registration — the token must be admin/owner-issued, workspace-scoped, single-use, and time-limited. Usernames created this way must satisfy §4.1's `[a-z0-9._-]{3,32}` format (not merely `max:64` like the legacy login/admin-creation paths, which predate that rule and were left as-is rather than retroactively tightened). Rejected: token issued without expiry (a leaked QR image would stay exploitable indefinitely); redemption creating a session without letting the invitee pick their own password (worse UX, and account recovery would need another admin-mediated step). | 2026-09-21 |
| DEC-082 | **System admin can upload/remove an installation-wide logo (`branding.logo_path`), and the browser tab title now shows the active workspace's name instead of a hardcoded product name.** Logo: a new `FileUpload` field on the existing System settings page (FR-ADM-009), stored on **local disk** — deliberately not the `public`/S3 disk, since this installation's `/storage/` route proxies MinIO (a `Storage::url()` on `public` would 404) and the per-workspace `Attachment`/`WorkspaceScope` pipeline has no meaning for a system-wide asset with no owning workspace. Served through two new PUBLIC (pre-auth) endpoints, `GET /api/v1/app-config` and `GET /api/v1/branding/logo` (API-234/235), since the mark must render on `/login` and the public `/join/:token` page before any session exists; the served URL is cache-busted by the setting row's own `updated_at` so it can be `Cache-Control: immutable`. Falls back to the built-in Banana mark on missing config OR image-load failure. **Title**: `unreadTitle()` (packages/chat-core) keeps FR-NOTI-007's unread math completely unchanged — N is still the SUM across every one of the user's workspaces — and only swaps the trailing label: the active workspace's name when one is selected (`(7) TryCatch`), the product name with no active workspace or logged out (`(N) Banana Chat` / bare `Banana Chat`). No admin-configurable app *name*, no favicon change — both explicitly out of scope for this pass; **partially closes OQ-010** (§17.1 — the logo/icon half; app name and color remain open). | Requested feature. UX/UI designed and reviewed with Codex CLI across three rounds (design brief → implementation review → final confirmation), which is also how a real functional bug was caught before shipping: the FileUpload field's Livewire state was never cleared after a successful save or logo removal, so an already-consumed (and by then possibly deleted) upload path could get written straight back into `branding.logo_path` on a later, unrelated Save — fixed by explicitly clearing that field's state in both `save()` and the `removeLogo` header action, with a dedicated regression test (TC-ADM-083) reproducing the exact sequence. Codex also caught a visual regression of its own recommendation: an early pass only turned the *inner* 32×32 logo slot white, leaving the outer 40×40/42×42 button's existing yellow background as a visible ring around an uploaded logo — fixed with a `:has(.bc-logo-mark.has-logo)` rule so the whole tile goes white, yellow reserved strictly for the built-in fallback. | 2026-09-22 |
| DEC-083 | **Forward (FR-MSG-011) leaves the Appendix C parking lot, Telegram-style: a forward is a NEW message sent by the forwarder that carries the original author as `forwarded_from`, with its own copies of every file.** (a) **Copy bytes, not rows**: an attachment can belong to only one message (FR-MSG-002) and five purge paths (message delete, moderation, secret-room expiry, room tools, kanban) delete by that row's `storage_key` + `derived` keys; sharing a row or a key would let deleting the source — or its secret room expiring — destroy the forwarded file. Each copy gets new objects (server-side S3 copy) under a new row owned by the forwarder. (b) **Metadata is written in the INSERT**, not after (unlike API-111 AI share), so the EVT-010 payload already carries `forwarded_from`; only that derived field is serialized, never raw `metadata`. (c) **No edit of forwards** (`MSG_NOT_EDITABLE`), **no mentions, no AI-bot trigger** from forwarded bodies. (d) **Secret rooms**: out is refused, in is allowed. (e) **Per-target commit**: each target is its own API-040 write transaction; no outer transaction spanning rooms (would take several room locks in arbitrary order and deadlock against normal sends). Failures after validation are reported per room in `failed_room_ids`. | Customer request 2026-09-26 ("forward like Telegram, keep the original sender"). Rejected: sharing attachment rows (breaks every purge path); forwarding by reference/link to the source (non-members of the source room cannot read it); letting forwarders edit (attributed fabrication); a chat-core outbox path (forward is an online action with no optimistic row in the current room, and N×M copies would need N×M outbox entries). Web UI forwards one message to many rooms; the API already accepts up to `message.forward_max_messages` for a later multi-select mode. | 2026-09-26 |
| DEC-084 | **The room bot's history (FR-AI-021) is a background transcript in the system prompt, and the mention is the only `user` turn.** Each earlier line is `Name: text`; the bot's own replies are `You (AI): text`; the block is headed as already-handled context the model must not reply to, summarise or comment on unless the question refers to it. The system prompt also tells the model the room talks about many unrelated things and it must answer only the addressing message. Budget and exclusions (deleted, system, attachments, memories) are unchanged. **1.16.2 follow-up:** the cap default drops 20 → 10 (`ai.room_bot.history_messages`, still admin-editable 0–200), and the bot's own **notices** (consent required, quota, busy, unavailable, failed, empty) are no longer context: new ones carry `metadata.bot_notice=true` (never serialized to clients), rows from before that are matched by exact text; a cut-short answer keeps its words but loses the trailing `[Answer truncated…]` / `[AI could not complete…]` note. | Customer report 2026-09-26: in a busy group the bot "talks all messages" — asked `@ai check credit`, it replied to the flood small talk, a flooding question and the credit request in one answer (staging room `01m2snbwrt…`, seq 245). Measured on staging: every recent mention reached the model as 17–20 unanswered `user` turns with 0–3 `assistant` turns between them, so "answer the last message" in one line of system prose lost to twenty questions. Rejected: keeping per-line turns with a stronger prompt (models still treat each `user` turn as addressed to them); dropping history (FR-AI-021 exists so follow-ups work). 1-1 AI chat (FR-AI-005) was checked and left unchanged: its history strictly alternates user/assistant (0 adjacent user turns, 0 failed replies on staging and prod). | 2026-09-26 |
| DEC-085 | **The room bot keeps a rolling per-room summary (`ai_room_summaries`) of lines it was already sent, injected above the raw lines as background.** When a mention's window is full (`count == ai.room_bot.history_messages`, cap ≥ 4), `CompactRoomContext` rolls the oldest half of *that mention's transcript* into the summary (non-streaming call on `memory_model`, clipped by the token estimator to min(1500, 15 % of input budget); the builder re-clips to 20 % of the current provider's budget). Raw lines then start after `up_to_seq`, so nothing is sent twice. **Consent scope**: only lines already sent in a mention transcript are summarised — no backfill of older history — and only the version that was sent (a line edited after the mention is skipped); the job re-runs the mention's `AiGate` (AI on, workspace allowed, requester consented). **Never outlives its sources**: no summary for secret rooms (they expire), or with `summary_enabled=false` or `history_messages=0`; the row is deleted when a covered message is edited or deleted (Message `updated` hook), when the room is soft-deleted (Room hook), by FK cascade on hard delete, and — as a backstop for writes that skip model events — by the builder and by the job before sending, both comparing covered rows' `deleted_at`/`edited_at` against `source_read_at`. Setting `ai.room_bot.summary_enabled` (default true). | Customer asked for the room bot to follow context like ChatGPT/Claude without drowning in it (2026-09-26); a summary lets a follow-up reach back past ten lines at a fraction of the tokens. Reviewed adversarially by Codex (8 findings; 7 fixed, all with mutation-tested regressions). **Accepted limits**: an edit/delete that lands while the summary call is in flight cannot be recalled from the provider (identical to the bot's existing raw-history behaviour); a compaction dispatched while another for the same room is still queued is dropped (`ShouldBeUnique`), which only narrows the summary. **Future work must keep this true**: `workspaces.message_retention_days` exists but nothing purges by it yet — whoever builds that purge must also drop summaries whose range it touches. Rejected: backfilling older history on first use (sends content the room never saw sent); sharing one timestamp across merged ranges without re-validation (either leaks or falsely invalidates). | 2026-09-26 |
| DEC-080 | **DEC-078's rail/sidebar/workspace chrome is AppShell's persistent frame, not a chat-surface concern, and now applies on every authenticated route** (`/`, `/rooms/*`, `/ai*`, `/search`, `/members`, `/meetings`, `/board`, `/public-chat`): rail (64px desktop / 56px tablet+mobile, `#25332e`), sidebar (336px ≥1200px, 280px 768–1199px, white background, `Banana Thai` font), workspace header spacing and the sidebar footer's hidden trailing icon move from `.bc-chat-shell` to `.bc-app` in `chat.css`. Only the five `--chat-*` custom properties move with them — never the `color: var(--chat-ink)` declaration DEC-078 paired them with, which would otherwise bleed into every non-chat page's main content. Pane styles (chat header, timeline, bubbles, composer, the mobile is-conversation/is-chat-list list↔detail switch) stay `.bc-chat-shell`-only: non-chat routes keep index.css's existing absolute-overlay drawer and `.bc-outlet{overflow:auto}` on phones, which the two-pane chat mechanics would break. No JSX/AppShell change; no API/schema change; `/public-chat`'s staff queue table itself is unchanged, only the sidebar frame around it. | Customer noticed the redesigned rail/sidebar on `/`, `/rooms/*` and `/ai*` but not on the five content pages, since AppShell always renders the rail+sidebar regardless of route while DEC-078 scoped its restyle to `chatSurface` routes only. Rejected: widening AppShell's `chatSurface` flag instead of rescoping CSS — `chatSurface` also gates `.bc-chat-shell .bc-main/.bc-outlet{overflow:hidden}` (would break scrolling on `/board`, `/meetings`, `/members`, which rely on index.css's `.bc-outlet{overflow:auto}`) and `.bc-chat-shell .bc-sidebar-shade{display:none}` (would remove the mobile drawer shade those same pages need) — the chrome and the pane mechanics needed independent scopes, not one shared flag. | 2026-09-20 |

---

## 16. Changelog

2026-09-19 — DEC-078 bubble refinement following user photo: short plain-text metadata inline, compact44px-minimum reply inset; synthetic Thai bubble geometry regressions. Full19 browser tests passed locally.

2026-09-19 — DEC-078 visual refinement: compact filter pills, softer chat surfaces, selected-row edge indicator, rounded composer/circular send, frameless mobile Back, self-hosted Noto Sans Thai (OFL) and app-locale message times. Full17-case browser suite passed; customer visual acceptance remains open.

2026-09-19 — DEC-078 / PLAN_ASTRA follow-up: web chat layout and contextual actions redesigned; cached room navigation survives refresh failure; quote highlight lifecycle, scoped history pagination and viewport anchors corrected. Added isolated Playwright UI regression suite. See `UI_REVIEW_ASTRA.md` for validation and remaining acceptance gates.
...
|---|---|---|---|
| 1.15.21 | 2026-09-23 | Claude | **Unconfigured AI now explains itself instead of inviting a chat that cannot send, and status can finally tell “no provider” from “workspace not allowed” (FR-AI-001, TC-AI-125, TC-UI-014; no DEC — implements an existing AC)**: FR-AI-001 AC#2 (§ line 1343) has always said that with no `is_enabled && is_default` provider the nav entry stays visible and opening it shows “ผู้ดูแลยังไม่ได้ตั้งค่า AI”. It was never implemented: `configured` was returned by API-100 and typed in `packages/shared`, but **no client code read it**. So an unconfigured instance rendered the ordinary sidebar empty state (`ai.empty` / `ai.emptyHint` — “no conversations yet / start a new chat”, an invitation to four actions that all 503), a working-looking “new chat” button whose blanket catch popped the consent dialog, and on `/ai/:id` a fully enabled composer; the user learned the truth only from a failed send, because `loadConversations` swallows the 503 in a bare `catch {}`. **Backend prerequisite, found by Codex and proven before fixing**: `AiController::status` derived both flags from `AiGate`'s provider, which is null for *either* reason, so a workspace-denied user reported `configured:false` + `allowed_in_workspace:true` — byte-identical to no-provider-at-all, and both flags wrong. A throwaway Pest run confirmed all three cases before any change. `status` now resolves `AiProvider::defaultProvider()` independently (the same `is_enabled && is_default` query the gate uses, so nothing else moves), computes `allowed` separately, and withholds the provider name/model from a workspace that may not use it. Side effect: `AiAssistantView`'s `ai.notAllowed` branch becomes reachable for the first time. **UI**: new shared `AiNotConfigured` component rendered in both the pane (before the memories/chat/deep-link branches, inside the pane so the header keeps its mobile back button) and the sidebar (ahead of search, cached rows and `ai.empty` — cached conversations can outlive a removed provider). Both surfaces carry it because the mobile shell shows exactly one at a time. New chat, memories and search are not rendered at all rather than disabled or `hidden`: nothing re-enables them until an admin acts, and the guard is `aiReady` (status known AND configured AND allowed) rather than `notConfigured`. `is_system_admin` is now typed on `Me` (`UserResource` always sent it; nothing could branch on it), so an admin gets an actionable hint plus a link to `/admin` while a member is told to ask one. Four new keys in both locales (en/th verified at identical 273-key sets). Requests are gated on positively-confirmed readiness (`configured === true`, not `!== false`) because `open()` → `loadMessages` awaits without a catch while its caller discards the promise, so a doomed call surfaces as an unhandled rejection. Codex's proposed loading/ready state machine was deliberately **declined** as a redesign of the store's loading model that AC#2 does not require. **Caught by looking, not by asserting**: the admin CTA was first styled `.bc-chat-control` — a 40px icon button — and screenshotted as one word per line overlapping the hint, with every test still green; it is now `.bc-primary` (explicitly supported as `a.bc-primary`, buttons.css:13) with a scoped rule. Codex round 2 returned FIX on three points, all taken: `startNewChat` was unguarded, so on a cold load — where `status` is still `null` and therefore *not* `notConfigured` — the button rendered and its 503 landed in a blanket catch as a **consent dialog**; `hidden={notConfigured}` leaned on Tailwind preflight's `[hidden]{display:none!important}` to beat these elements' own `display:flex` (verified present, but load-bearing and undeclared); and it missed the workspace-denied case entirely, which reports `configured:true` and so kept every failing control on screen. Render-gating on `aiReady` fixes all three at once. Codex round 3 then found that applying those fixes had introduced a regression into the **healthy** path, independently caught by review: gating `loadConversations` on readiness left `loading` false while `/ai/status` was in flight, so the nav body hit `!loading && conversations.length === 0` and rendered `ai.empty` — “no conversations yet / start a new chat” — for one RTT on every normal cold load, to a returning user with a full cached list no less (the cache hydration lives inside `loadConversations`). Instant mocks cannot see this, so the new test holds `/ai/status` open 900ms and checks `ai.empty`'s count **point-in-time** rather than with the auto-retrying matcher, which would simply have waited out the delay and passed regardless. The skeleton now covers unknown status, `ai.empty` requires readiness, a denied workspace gets `ai.notAllowed` instead of falling through to the same invitation, and the search branch is readiness-gated because a stale `debounced` term survives a workspace switch after its input unmounts. A further one-render-frame transient remains impossible to observe from a test — readiness flips during render while `loadConversations` cannot set `loading` until the effect runs after commit — so an `everLoaded` flag closes it as a defensive guard. It is one of the two guards here that are *not* mutation-tested — its window is sub-frame, and the readiness gate on the search branch would need a workspace switch performed mid-search to observe; both are recorded as defensive rather than proven. Verified: TC-AI-125 asserts all three status cases and is mutation-tested (reverting the controller reddens it on the `configured` assertion); TC-UI-014's six cases cover 390px/1440px across both `/ai` and `/ai/:id`, admin vs member, workspace-denied, slow-status, and a configured-instance control, and are mutation-tested twice — forcing `notConfigured=false` reddens the three unconfigured cases while the control stays green, and reverting the control guard from `aiReady` to `notConfigured` reddens *only* the denied case, which is precisely the gap it was added for. Its first 390px assertion used `.first()` and failed correctly, having asserted DOM order and picked the hidden sidebar copy on a deep link. `apps/api/tests/Feature/Ai` 78 passed/1 skipped, e2e UI 29/29, web typecheck and lint clean, production Vite build green. Four mutation tests in all: reverting the controller reddens TC-AI-125; forcing `notConfigured=false` reddens the three unconfigured UI cases while the configured control stays green; reverting the control guard to `notConfigured` reddens *only* the denied case; and reverting the skeleton/empty conditions reddens *only* the slow-status case. **Found but NOT fixed**: FR-AI-001 AC#1 says the nav entry should be hidden when `ai.enabled=false` or the workspace is not allowed — `AppShell.tsx:132` never gates it and does not read AI status at all (separate change, own tests). **Caught only by the post-deploy check on prod**: TC-UI-014's `getByLabel('แชทใหม่')` matched nothing — `ai.newChat` is “บทสนทนาใหม่” — so every `toHaveCount(0)` against the new-chat button passed vacuously, and the hiding was really only proven by the neighbouring search-placeholder assertion. The prod browser check reported the button invisible on a healthy instance, which is what exposed it. All five Thai literals in these tests now reference `packages/shared/i18n/th.json` directly, where a wrong key is a compile error rather than a silently green test, and the configured-instance case asserts the controls are **visible** so that a locator matching nothing can never again satisfy the negative assertions. TC-UI-007 (typing subscription over the mocked websocket) flakes at roughly 1 in 10 — `expect “มินตรา Chen”, received “”` — and touches none of this code. Proven pre-existing rather than assumed: reverting both e2e files to clean `main` reproduced the identical failure at 1 in 10 before restoring. |
| 1.16.3 | 2026-09-26 | Claude | **Room bot rolling summary (FR-AI-021, DEC-085)**: new table `ai_room_summaries` (migration `2026_09_26_000001`), job `CompactRoomContext`, setting `ai.room_bot.summary_enabled`. Tests (11 new in `RoomToolsTest`): rolls only already-sent lines; summary above raw lines with covered lines not repeated; secret room none; delete/edit drop it; mass-update backstop; moderated room; edited-after-sent excluded; AI gate re-run; history 0 → mention alone; Thai clip. Each guard mutation-tested (removing it reddens its test). Full suite 624 passed / 1 failed (TC-NOTI-026, pre-existing). **Deploy needs `php artisan migrate --force`.** |
| 1.16.2 | 2026-09-26 | Claude | **Room bot context trimmed (FR-AI-021, DEC-084 follow-up)**: `ai.room_bot.history_messages` default 20 → 10 (no stored override on staging/prod, so the default applies); bot notices tagged `metadata.bot_notice` and excluded from the next mention's context (legacy rows by exact text); trailing truncation/failure notes stripped from the bot's own earlier answers. Tests: busy-room cap, tagged + legacy notice, stripped note, `stripNotes`/`isNotice` unit; the two behaviour tests fail on the 1.16.1 builder. Full suite 613 passed / 1 failed (TC-NOTI-026, pre-existing). |
| 1.16.1 | 2026-09-26 | Claude | **Room bot answers only the message that called it (FR-AI-021, DEC-084)**: room history moved from ~20 separate `user` turns into a background transcript in the system prompt; the mention is the single `user` turn. Fixes the bot replying to every recent line in a busy group. Tests: FR-AI-021 block in `RoomToolsTest` rewritten for the new shape plus a busy-room case; all four fail on the old builder. Note: that block had been failing on every local run because the api container exports `QUEUE_CONNECTION=redis`, which beats phpunit.xml's forced `sync` — run Pest with `-e QUEUE_CONNECTION=sync` as well as `DB_DATABASE`/`CACHE_STORE`. Full suite 610 passed / 1 failed (TC-NOTI-026, identical on main). |
| 1.16.0 | 2026-09-26 | Claude | **Forward messages (FR-MSG-011, API-047, DEC-083)**: `POST /messages/forward` copies messages from one room into up to `message.forward_max_rooms` rooms, each copy sent by the forwarder with `forwarded_from` (original author, chain-collapsing); attachments copied to new storage objects; secret source refused, secret target allowed; forwarded messages not editable, no mentions/bot trigger; idempotent via `client_forward_id`; audit `message.forwarded`. New settings `message.forward_max_messages`/`message.forward_max_rooms`, error `MSG_FORWARD_INVALID`. Web: Forward action + multi-select room picker + "Forwarded from" header; mobile renders the header. TC-MSG-061..071, TC-WEB-078..080. |
| 1.15.20 | 2026-09-23 | Claude | **AI chat composer input rendered at 22% width — fixed and guarded (FR-AI-003, DEC-078, TC-UI-013; CSS/markup only, no DEC)**: on `/ai/:id` the composer's textarea measured 183×61 inside its 832×75 pill (22.0% fill) with `flex-grow:0`, a visible `resize:vertical` grabber, and a pill 19px taller than the room's — the field hugged the left edge with the send button stranded mid-row and ~75% of the pill empty. Root cause: `.bc-compose-box textarea` (index.css) never owned `flex`/`resize`; the room composer only looked correct because `Composer.tsx` carries `flex-1 resize-none` as Tailwind utilities on the element itself, and `AiChatPane.tsx`'s textarea carries no className at all. The same DEC-078 note that says the AI pane "reuses the chat shell's composer classes" is exactly what made this invisible — reusing the class got the pill, not the field inside it. Fixed in CSS rather than by adding utilities to the AI element, matching the in-repo precedent `.bc-pchat-compose-box textarea` (public-chat.css:189), which has always declared `flex:1; resize:none` itself — so the public chat composers, whose textareas are likewise className-less, were never affected. Verified no-op for the room composer (87.3% fill before and after, byte-identical geometry) since its utilities already said the same thing. Also aligned `rows={2}` → `rows={Math.min(5, draft.split('\n').length)}` (matching `Composer.tsx` exactly), which closes the remaining 75px-vs-56px height gap and lets the field grow 1→5 rows, and gave the textarea an *explicit* accessible name (it previously had none of its own and fell back to the placeholder). UX/UI designed and reviewed with Codex CLI over three rounds per this task's process requirement. Round 1 independently confirmed the root cause, chose dynamic rows over keeping `rows={2}` ("the explicit request for consistency wins"; public chat's own `rows={2}` is a different component, not the precedent here), and confirmed the send button, disabled state, focus ring and mobile overrides needed no change. Round 2 verified the applied patch and caught two real gaps: the first version of TC-UI-013 asserted only geometry, so reverting either JSX change still passed green — now covered by two further tests asserting rows 1→3→5 and the `aria-label` in both consent states. Round 3 then caught that even those were under-strength, twice over: `toHaveAccessibleName(/.+/)` is tautological because a bare `placeholder` already supplies an accessible name, and a merely *non-empty* `aria-label` still passes if the conditional is reverted to a fixed string. Both assertions are now `aria-label === placeholder`, compared against the live attribute rather than a literal (the ui fixture runs in Thai), with the un-consented state — reachable via a new `installChatFixture(page, { aiConsented: false })` option that defaults to `true` so the 19 pre-existing tests are untouched — as the case that separates a conditional label from a fixed one. All three halves were mutation-tested: reverting `rows` to 2, deleting the `aria-label`, or pinning it to `text('ai.placeholder')` each turns a test red, the last failing *only* the consent case with `Expected "ต้องให้ความยินยอมก่อนใช้ AI` / `Received ถามอะไรก็ได้...` — i.e. the exact defect: a screen reader announcing "ask anything" on a composer that cannot be typed in); and a fixed `aria-label` would have *overridden* the placeholder as the accessible name, hiding the consent-required instruction from a screen reader while sighted users could still read it — the label is now the same conditional string as the placeholder. Result: AI composer 92.5% fill, `resize:none`, pill height 56px, identical to the room (the remaining 5pt width difference is the room's 38px attach button plus the pill's 6px gap, which the AI composer has no equivalent of — `chat.css:147`; the 44px figure is the ≤767px touch-target override at `chat.css:248`, which is why the same test's 390px case needs a lower floor). **Found but NOT fixed (pre-existing, out of scope)**: `AiChatPane.tsx`'s `onSubmit` guards only empty content, so pressing Enter while an answer is streaming still calls `send()` and clears the draft before the backend rejects it with `AI_GENERATION_IN_PROGRESS` — `canSend` already encodes the right condition (`liveStream === null`) but is only applied to the button, not the Enter key. Verified: e2e UI suite 23/23 (19 pre-existing + 4 new TC-UI-013 at 390px/1440px/rows/consent), production Vite build green with the built bundle carrying `.bc-compose-box textarea{resize:none;...;flex:1}`, web typecheck clean, lint unchanged (warnings only, all pre-existing and in files this change doesn't touch), e2e regression 4/5 with the one failure being the long-documented AI-assistant flake — proven pre-existing again this session by reverting both source files to clean `main` and reproducing the identical `TimeoutError ... getByTestId('ai-composer-input')` before restoring. |
| 1.15.19 | 2026-09-22 | Claude | **New `/admin/public-chat-integration` page — a concise Public Chat integration guide for humans and AI agents (FR-ADM-016, documentation only, no DEC)**: `PublicChatIntegration` Filament page (System group, "Public Chat guide") summarises choose-your-tier guidance, before-you-start, the 5-step ticket-to-conversation flow, one full worked API-200 request/response (host derived from `config('app.url')`, so it reads correctly on both staging and prod), the HMAC canonical string, and the 6 most-hit error codes — with two header `Action`s streaming the exhaustive, code-verified `apps/api/resources/docs/public-chat/README.md` and `openapi.yaml` directly, rather than re-typing their content, so there is exactly one place each fact can drift. **Infra fix required to make this possible**: the guide's whole tree (32 files: `README.md`, `examples/`, `bruno/`) moved from repo-root `docs/integration/public-chat/` to `apps/api/resources/docs/public-chat/` — the prod build context is `apps/api` (`infra/docker-compose.prod.yml` + `Dockerfile.prod`'s `COPY . .`), and repo-root `docs/` is outside it, so a page trying to serve the un-moved file would 404/500 in prod despite working on a dev box with the whole repo mounted. UX/UI designed with Codex CLI (page structure, content split, decision to download-not-duplicate) and independently verified against the running code before and after implementation. **Four accuracy fixes applied to the moved guide itself** (prose only, verified against the actual controllers/services, not the aspirational spec — same "code is the source of truth" discipline as 1.15.2): (1) §5.1's "customer_name/provider_name truncated to 120 chars, silently" was backwards — `PublicChatPartnerController::store()`'s `max:120` validation runs BEFORE the sanitiser and rejects anything over 120 with `422`, so the sanitiser's own truncate-to-120 branch is unreachable through this API (stripping/collapsing whitespace can only shorten a string, never grow it past a validated-≤120 input); (2) §3.2's "a room cannot sit in any status other than `new` with a null assignee" is true only of the agent transition surface (API-224 / FR-PCHAT-009's own guard) — `PublicChatService::close()` (API-202, partner) sets `status=done` directly with no assignee check, so an unclaimed room closed by the partner is `done` with `assigned_to: null`, which is normal, not a bug; (3) §3.3's HMAC flowchart's "all four headers well-formed?" node only actually validates the *shape* of `X-PChat-Key`/`Timestamp`/`Nonce` — a malformed `X-PChat-Signature` is never format-checked there, it reaches the HMAC comparison and fails as `401 API_SIGNATURE_INVALID`, not `API_KEY_INVALID` (confirmed against `VerifyPublicChatSignature`'s own docblock, which states this exception explicitly); (4) §5.1.1/§6's "a live socket cannot outlive the link" overstated the guarantee — expiry blocks *new* HTTP requests and channel re-auth (`PublicChatVisitorController::resolve()`), but nothing evicts an already-subscribed Reverb channel: `PublicChatMessageWriter::broadcastMessage()` has no expiry check, and an agent CAN still write to an expired-but-open room (only `isClosed()` gates agent sends, not `isExpired()`), so an already-open visitor tab keeps receiving live messages past its link's `expires_at`. **Note for a future task**: finding (4)'s exact phrasing ("a live socket cannot outlive the link") also appears verbatim in FR-PCHAT-012 and in `PublicChatVisitorController::broadcastAuth()`'s docblock — both left untouched here (out of this task's scope, and this task's verification was static code reading, not a live multi-client socket test) but worth a dedicated look. **A second, separate discrepancy surfaced by Codex CLI's round-2 verification**: `PublicChatAgentController::markRead()` (API-228) has no `$this->gate->assertEnabled()` call, so it is NOT actually gated by `publicchat.enabled` — but FR-PCHAT-034 explicitly lists "read pointer" among the agent writes that should 503 while the switch is off. Confirmed by reading the method directly (no gate call anywhere in it, unlike every sibling agent-write method). Not fixed here — it's a behavioral code-vs-spec question (silently 503-gating a previously-ungated endpoint is itself a behavior change needing its own DEC and test), not a documentation fix; this page's own "Before you start" copy was reworded to stop implying every write is gated, pending that decision. **Pre-existing regression found and fixed in passing**: `tests/Feature/PublicChat/AdminTest.php`'s two `FR-PCHAT-033` tests (unrelated to this task, last touched before FR-ADM-015 shipped in 1.15.18) were red — both independently iterate `SettingsService::DEFAULTS` and didn't know about the `branding.logo_path` exception `AdminExpansionTest.php`'s `TC-ADM-071` already carries; same one-line skip applied to both, now 193/193 across `tests/Feature/Admin` + `tests/Feature/PublicChat`. Verified: 4 new Pest tests (TC-ADM-087..090 — page renders, both downloads stream the real moved files, the common-errors table's codes are asserted against reality so a renamed code fails the test), Pint clean, full Admin+PublicChat suites 193/193. |
| 1.15.18 | 2026-09-22 | Claude | **Admin-configurable system logo, and the browser tab title now follows the active workspace (FR-ADM-015, DEC-082, API-234/235)**: new "Branding" section (a dedicated `FileUpload` field, not the settings page's generic per-key auto-form) lets a system admin upload/replace/remove the installation's logo — PNG/JPEG/WebP, 2MB cap, stored on local disk under `branding/` (never the `public`/S3 disk: this app's `/storage/` route proxies MinIO, and the per-workspace `Attachment`/`WorkspaceScope` pipeline has no meaning for an asset with no owning workspace). Two new public, pre-auth endpoints (`GET /api/v1/app-config` → `{logo_url}`, `GET /api/v1/branding/logo` → the bytes, `Cache-Control: immutable` cache-busted by the setting's own `updated_at`) since the mark has to render on `/login` and the public `/join/:token` page before any session exists. New `Logo.tsx`/`useAppConfig.ts` render the uploaded image (object-fit: contain, no crop, its own white tile — yellow stays reserved for the built-in Banana fallback) in place of the hand-drawn mark on the rail button, mobile top bar, login wordmark, and `JoinInvitePage`'s header, falling back to the Banana SVG on missing config OR image-load failure without changing layout. Separately, `unreadTitle()` (`packages/chat-core`) now takes the active workspace's name and uses it as the title's trailing label instead of a hardcoded "Banana Chat" — the unread count itself is completely unchanged (still the cross-workspace sum per FR-NOTI-007), so `(7) TryCatch` replaces `(7) Banana Chat` only when a workspace is actually selected; logged out or no active workspace still falls back to the product name. UX/UI designed and reviewed with Codex CLI across three rounds (design brief → implementation review → final confirmation) per this task's process requirement. Round 1 fixed the exact title-count semantics (keep the cross-workspace sum, swap only the label) and locked the per-surface sizing/tile rules. Round 2 caught a real functional bug: the FileUpload field's Livewire state was never cleared after `save()` or the `removeLogo` action, so uploading a logo, saving, removing it, then saving again with no new file picked would write the already-deleted path straight back into the setting — fixed by explicitly clearing that field after both actions, with a dedicated regression test (TC-ADM-083) reproducing the exact sequence live. Round 2 also caught a CSS miss in an early pass: only the inner 32×32 logo slot went white, leaving the outer 40×40/42×42 button's pre-existing yellow background as a visible ring around the uploaded mark — fixed with `.bc-brand:has(.bc-logo-mark.has-logo){background:#fff}` in both `chat.css` and `index.css` (needed in both: the rail button is `.bc-app`-scoped at 40px, the login wordmark's `.bc-brand` is the unscoped 42px base rule, and neither alone covers both contexts). Round 3: SHIP. Verified: 8 new Pest tests (TC-ADM-071a, TC-ADM-080..086 — upload/serve/cache-bust, remove deletes the file and reverts both endpoints, the exact stale-field repro, replacing a logo deletes the old file, the header action hides itself with nothing set, audit trail), the pre-existing TC-ADM-071 ("every setting has a form field") updated with a documented exception for this one file-backed key, plus 45/45 across the full Admin test files with dev data confirmed untouched (`docker compose exec -T -e DB_DATABASE=orgchat_test ...`, the isolation fix documented in 1.15.17). Frontend: chat-core vitest 222/222 (2 new `unreadTitle` cases), web typecheck/lint clean (no new warnings beyond the pre-existing baseline), e2e regression 5/6 (the 1 failure is FR-AI-002/003's pre-existing, unrelated-to-this-feature flake, reproduced identically on a clean `main` with zero branding changes present via a stash round-trip), e2e UI suite 19/19 clean on a second run (one transient flake on the first run reproduced as passing in isolation, confirming it wasn't a regression). |
| 1.15.17 | 2026-09-21 | Claude | **Owner/admin invites new members via a one-time QR/link (FR-AUTH-008, FR-WS-006, DEC-081, API-230..233)**: NG3 amended (not deleted, same discipline as DEC-060/061) to carve out exactly an owner/admin-issued, single-use, 24h-expiring invite token — no general public sign-up. New `workspace_invites` table (`token_hash` sha256, never plaintext, unlike `public_chat_rooms.code`); `WorkspaceInviteService::issue/revoke` (permission check inlined, matching `RoomPolicy::isWsAdmin`'s style since no `WorkspacePolicy` class exists); `RedeemInviteAction` does the atomic `lockForUpdate()` claim, creates the `User`+`WorkspaceMember` row, and logs the invitee in via `LoginAction::establishSession()` (reused directly, not duplicated) — response shape matches `/auth/login` exactly. Frontend: new "เชิญสมาชิกผ่าน QR" account-menu item (owner/admin only, `currentWorkspace.role` gate) opens `InviteQrDialog` (explicit "generate" step, not auto-issued on open; QR rendered client-side via the new `qrcode` dependency; live expiry countdown; copy-link; revoke-with-confirmation; the current invite is kept in `AppShell` state per workspace so reopening the menu shows the still-live invite instead of minting a new one, and is revalidated against the public preview endpoint on every reopen so a since-consumed-or-revoked invite never renders as falsely scannable) and a new public `/join/:token` page (outside `<EchoProvider>`/`<CallProvider>`, matching `PublicChatVisitorPage`'s established placement) with a registration form, browser-locale detection (there is no signed-in user yet to read a locale from, and the created account's `locale` now matches whichever language the form was actually filled in, rather than a hardcoded default), and per-field error mapping for all six invite/auth error codes. Username creation on this new path enforces §4.1's `[a-z0-9._-]{3,32}` format via a real validation rule — the legacy login/admin-creation paths only ever had a `max:64` check and a Filament helper-text hint, never an enforced regex; this new surface does not inherit that gap. UX/UI consulted and reviewed with Codex CLI per this task's process requirement, across two rounds: first pass caught a CSS specificity bug on the primary button (`.bc-message-menu button` (0,1,1) beat a bare `.bc-invite-primary` (0,1,0), fixed by scoping under `.bc-message-menu .bc-invite-primary` — the same pattern `.bc-message-menu .is-destructive` already uses) and non-localized expiry units ("23h 59m" mixed into Thai copy); second pass required the reopen-revalidation behavior above, explicit revoke-failure messaging (was silently falling back to "ready" as if nothing happened), the locale-consistency fix, accessible field-error wiring (`aria-describedby`, focus-on-invalid-field), and the stricter username format. Verified: 17 new Pest tests (issue/revoke permission gates, cross-workspace 404, preview is non-consuming, single-use enforcement, weak-password/duplicate-username/format rejection, locale round-trip) plus a genuinely concurrent proof — 4 real simultaneous HTTP connections against the same token, exactly 1 succeeded, 3 got `409 INVITE_ALREADY_USED`, 0.28s wall-clock (not Pest's sequential-only in-process client), dispatched across the dev API's real 8-worker `php -S` pool (`PHP_CLI_SERVER_WORKERS=8`, confirmed via 9 distinct PIDs in `ps aux` — genuine multi-process concurrency, not one listener queueing requests) — confirming `lockForUpdate()` race-safety under actual concurrency, not just replay-after-the-fact. Manual E2E: full desktop + 390px flows for issue → QR → copy → revoke-confirm → keep → reopen-shows-same-invite, a plain `member` (two different accounts) confirmed NOT seeing the menu item, a brand-new visitor scanning through to registration → auto-login → landing in the workspace on mobile, and — specifically targeting the reopen-revalidation fix — an invite issued in one browser tab then revoked out-of-band from a second, independent session (bypassing the UI's own revoke path entirely, so the first tab's in-memory cache stays genuinely stale): reopening the dialog in the first tab does not render the now-invalid QR, correctly shows the revoked state, and offers "generate" again. **Separately discovered and reported**: the Makefile's own `test` target (`db-test` → `cd apps/api && ./vendor/bin/pest`) runs Pest **on the host** against the host-mapped Postgres port (`127.0.0.1:5433`), which is exactly why `phpunit.xml` pins `DB_HOST=127.0.0.1`/`DB_PORT=5433`/`DB_DATABASE=orgchat_test` with `force="true"`. This task instead invoked Pest via `docker compose exec api ...` (inside the container) as a matter of habit — and `docker-compose.yml`'s `api` service `environment:` block sets `DB_DATABASE=orgchat`, `CACHE_STORE=redis`, `QUEUE_CONNECTION=redis` in the container's OS environment. Laravel's `env()` reads `$_SERVER`/`getenv()` before honoring PHPUnit's `force="true"` (which only calls `putenv`/`$_ENV`), so the container's own env wins and `RefreshDatabase` silently ran against the live `orgchat` database instead of `orgchat_test` — wiping seed data twice during this task, both times caught immediately and restored via `migrate:fresh --seed`. Root-caused and confirmed fixed in-session: passing `docker compose exec -T -e DB_DATABASE=orgchat_test -e CACHE_STORE=array -e QUEUE_CONNECTION=sync -e SESSION_DRIVER=array api ./vendor/bin/pest ...` (explicit overrides at the `exec` layer, which do take precedence) resolves to `orgchat_test` and leaves `orgchat`'s row count unchanged before/after a full run of this feature's 17 tests. Not a code change in this feature's scope — flagged to the user as a docs/tooling gap: running Pest inside the container needs either these explicit `-e` flags or, simpler, always using the Makefile's host-side `make test`. |
| 1.15.16 | 2026-09-21 | Claude | **Read receipts for DM and group chat (FR-READ-002, FR-UI-MB-003), plus bottom-anchoring the message actions toggle (FR-UI-MB-004, DEC-078)**: DM now shows "อ่านแล้ว" under our own latest message when the other member's `last_read_seq` reaches it; group shows "อ่านแล้ว N คน" as a tappable trigger opening a reader-list dialog (avatars + names), both reusing the already-implemented, room-type-agnostic `GET /rooms/{id}/read-status` endpoint (API-046) — no backend change was needed, only enabling the existing query for group rooms and rendering `read_by` minus self. New `ReadReceiptTrigger`/`ReadReceiptDialog` (`ReadReceipt.tsx`): the trigger is stateless and safe to unmount per-message, while the dialog's open/entries state is lifted to `ChatView.tsx` and rendered once outside the message `.map()`, so sending a new own message (which changes which message is "latest" and unmounts that message's trigger) cannot also unmount an already-open reader dialog — verified by opening the dialog then posting a message via a same-page authenticated `fetch()` (simulating another tab) and confirming the dialog kept its frozen snapshot. The reader dialog's backdrop click uses the same bounding-rect guard as `MessageItem`'s existing action menu, not a bare `target === currentTarget` check, so clicking the dialog's own padding doesn't close it. Separately, `.bc-message-actions-toggle` moved from vertically centered (`top:50%`) to bottom-anchored (`bottom:0`) so it stays reachable on long/multi-line bubbles; grouped consecutive touch-target spacing increased from 4px to 6px at `@media (pointer:coarse)` so two stacked 44px targets don't approach overlap. UX/UI consulted and reviewed with Codex CLI per this task's process requirement — final verdict **SHIP** after two rounds fixed the dialog-unmount issue above and a WCAG contrast failure (both the new group trigger's `#8a94a0` and the pre-existing DM "Seen" `text-slate-400`, 2.84:1/2.36:1, switched to `var(--chat-muted)` at 4.69:1). Verified: chat-core vitest 221/221, mobile jest 45/45, e2e regression 6/6, e2e UI suite 19/19, plus a dedicated mobile (390px) run confirming the trigger, bottom-sheet dialog, Escape-to-close, and composer re-enabling after close. |
| 1.15.15 | 2026-09-20 | Claude | **Fix the in-room `@ai` disclosure banner breaking mid-word on phones (FR-AI-021, DEC-078)**: `.bc-chat-ai-notice a`'s `max-width: 60px` in the mobile media query was sized for the English label ("AI & privacy") and forced the Thai label ("AI และความเป็นส่วนตัว") to wrap inside itself, splitting "ความ" from "เป็นส่วนตัว" onto separate lines disconnected from the rest of the banner. Deleted the fixed width and added `white-space: nowrap` to the link plus `min-width: 0` to the message span, so only the message wraps and the link always moves as one unbroken unit. Verified at 320/390/768/1440px; UI suite 19/19 (TC-UI-001/012 overflow gate at 320/390/1440 with a `type:'group'` fixture, so the notice is exercised); regression `login → Engineering room` 1/1 on real data. Only `.bc-chat-ai-notice` rules changed. |
| 1.15.14 | 2026-09-20 | Claude | **Redesigned rail/sidebar/workspace chrome now applies everywhere, not just chat surfaces (DEC-078/DEC-080)**: customer noticed `/search`, `/members`, `/meetings`, `/board` and `/public-chat` still showed the old rail (72px, `#29372f`) and sidebar (300px, off-white, default font) alongside an already-redesigned room list, because DEC-078 scoped the chrome to `.bc-chat-shell` (only `/`, `/rooms/*`, `/ai*`). Rescoped `chat.css`'s rail/brand/rail-links/sidebar/workspace/sidebar-footer rules (and their 768–1199px and mobile-width variants) from `.bc-chat-shell` to `.bc-app`, which every authenticated route renders; only the five `--chat-*` custom properties move with them, not the `color` declaration DEC-078 paired them with, to avoid bleeding into main-content styling on those five pages. Sidebar's 336px width is wrapped in `@media (min-width:1200px)` so it can't reach index.css's ≤760px overlay-drawer breakpoint (336+64 would overflow a 390px phone). No JSX or AppShell change: `chatSurface` stays exactly as scoped, since widening it would also have hidden the mobile drawer shade and disabled scrolling on `/board`/`/meetings`/`/members`. `/public-chat`'s queue table itself is unchanged. Verified: build clean; 6/6 regression + 19/19 UI suite passed (chat pages are byte-identical in effect — same specificity, same source order); manual before/after screenshots at 1440px and 390px for all five routes plus `/`. |
| 1.15.13 | 2026-09-19 | Claude | **AI Assistant redesigned onto the shared chat shell (FR-AI-001/007/020, DEC-078/DEC-079)**: `/ai*` now uses one sidebar — `AiConversationList` (new, styled like `RoomList`) replaces `RoomList` in `AppShell` for that route — and the pane reuses DEC-078's `bc-message`/`bc-message-bubble`/`bc-composer` classes instead of a separate Tailwind-only layout; search (FR-AI-020) and send/regenerate/edit-resend/version-switch/share are unchanged. Memories (FR-AI-007) moved from a client-only toggle to a real `/ai/memories` route after manual testing found it unreachable on mobile: `.bc-chat-shell.is-chat-list .bc-main{display:none}` hides the outlet at `/ai` on a phone and the toggle changed no `AppShell` class, so nothing showed the panel — the same failure mode independently found and fixed for the consent dialog, which now portals to `document.body` (matching `MessageItem`'s existing dialog pattern) instead of rendering inside the pane it can be hidden with. Also fixed during verification: conversation rows were rendering as inline 40px pills, not stacked list rows — buttons.css's generic button-geometry rule (specificity 1,1,1) beat `.bc-conversations .bc-room-row` (0,2,0) because the rows were `<button>`s; switched to `NavLink` (matching `RoomList`) and added the row class to buttons.css's existing `:not(:where(…))` opt-out; the composer's quota/usage counter was invisible (`.bc-compose-hint{display:none}` on mobile, unguarded) — moved it to its own `.bc-ai-compose-meta` class; the empty-conversation prompt and the memories panel's scroll region got the `flex:1`/`min-h-0` their flex-column parents need. **Local-only, not deployed**: the AI regression test needs an `ai_providers` row at `http://mock-ai:8787/v1` (model `mock-glm`) and `AI_ALLOW_PRIVATE_HOSTS=true` in `apps/api/.env`, or the DEC-075 SSRF guard blocks the docker-internal mock provider host and the suite reports 5/6 instead of 6/6 — see the note in `e2e/regression/web.spec.ts`. Full 6/6 regression + 19/19 UI suite passed after each round of fixes. Public Chat's staff queue intentionally out of scope for this pass — full-width filterable table by its own design, not a sidebar+list surface. |
| 1.15.12 | 2026-09-18 | Claude | **Account menu really fixed, verified against the running app**: 1.15.11's specificity fix could not have worked. `buttons.css` pins rail buttons with `#root :is(.bc-rail-links button, .bc-rail-bottom button, …)` — **(1,2,1)**, and an ID always beats the (0,3,1) class selector that was fighting it, so both items stayed 40–44px squares with `padding:0` and sat side by side. Fixed structurally: the rail icon geometry (index.css base, mobile and hover, plus the buttons.css touch-target rule) now targets **direct children** (`.bc-rail-bottom > button`), so a dropdown nested in the rail is no longer treated as a rail icon, and `.bc-account-menu button` / `.bc-account-trigger` are added to buttons.css's `:not(:where(…))` opt-out — the mechanism that file already provides, which costs no specificity. **Verified on the deployed app rather than a mock**: the earlier check used hand-written HTML with the compiled CSS, which did not reproduce the real cascade and wrongly reported success. Playwright now authenticates by seeding the `orgchat.refresh` token from an API login (the login form's inputs never become visible to automation), opens the real menu at 390px and measures it: items 186×40, stacked 741→781 and 781→821, `overlap:false`. Note for future UI checks on this box: only bold text renders in this headless Chromium, so a blank screenshot is a missing regular-weight face, not a CSS fault — confirm with `textContent` and geometry, or inject a system font for the screenshot. |
| 1.15.11 | 2026-09-18 | Claude | **Fix account-menu items overlapping on phones** (superseded in part by 1.15.12 — the first attempt chased specificity and could not work): the menu lives inside `.bc-rail-bottom`, whose `.bc-rail-links button,.bc-rail-bottom button` rule pins every descendant button to a 43px grid cell — and the mobile media query re-pins it to **37px later in the file at equal specificity (0,1,1)**, so `.bc-account-menu button` won on desktop and lost on phones. "Change password" and "Sign out" were both squeezed into a 37px box and rendered on top of each other. Rules are now scoped through `.bc-rail-bottom` (0,2,1) so they win inside and outside the media query, and restate `width`/`height`/`display` because the rail rule sets each. A first attempt using `min-width:0; width:max-content` for the phone case collapsed the panel to ~36px — the containing block is the ~37px `.bc-account` cell and `max-content` did not expand past it — so the mobile override keeps a real `min-width:200px` with `max-width:calc(100vw - 72px)`. Verified by rendering the real compiled CSS at 360/390/1440px and measuring: no overlap, no clipped text, not off-screen at any width. |
| 1.15.10 | 2026-09-18 | Claude | **Account menu so users can actually FIND change-password (FR-AUTH-004)**: 1.15.8 added a voluntary entry point but customers still reported they could not change their password. The cause was discoverability, not availability — it was an unlabeled lock icon at the bottom of a 56–72px dark rail beside Sign out, with only a `title` tooltip, which never appears on touch at all. Replaced with an account menu opened from the user's own avatar (the convention people already reach for), showing the display name and handle plus **text-labelled** Change password and Sign out items, dismissible by outside click and Escape. The standalone Sign out button stays in the rail for anyone used to it. The menu opens to the right of the rail and is bottom-anchored, because anchoring below a trigger that already sits at the bottom of the viewport would push it off-screen. Mobile needed no change: its entry is a labelled row under Settings, which was already discoverable. |
| 1.15.9 | 2026-09-18 | Claude | **Mobile parity wave 1 (TASK-MOB-006/007/009)**: closes the gaps a mobile user hits in the first five minutes. Every item is a pure client build against endpoints already compiled into the mobile bundle — no backend work. **Message actions**: long-press opens a sheet with Reply / Edit / Delete (the last two only on your own messages, since offering an action guaranteed to 403 is worse than hiding it); the composer gains reply and edit modes, and bubbles now render the quoted parent, an `edited` marker and a proper deleted tombstone. An edit is a direct PATCH rather than an outbox entry **on purpose** — the outbox exists so a NEW message survives being offline, and replaying an edit against a body that has since moved on would silently clobber someone else's change. **Attachments**: the upload pipeline (`createUpload` → PUT → `completeUpload`) already existed in `outbox-flusher` and `expo-image-picker`/`expo-document-picker` were already installed, but nothing imported them, so a mobile user could never attach anything; adds a picker, a staged-file strip and inline image rendering (`thumb_md` first — the original can be many MB and the list is inverted and virtualised). Attachment-only messages are allowed, matching §10. **Member directory + start a DM** (`app/members.tsx`): mobile previously had **no way to begin a conversation at all** — `rooms.tsx` could only open a room that already existed. **In-app notification centre** (`app/notifications.tsx`, FR-NOTI-006 / API-073): rows have been written server-side since PH2 and rendered on web since PH4, but a mobile-only user could not see mentions, room invites or ticket-due alerts unless a push happened to arrive. 24 new i18n keys in both en and th (key sets verified identical). Cover: mobile tsc clean, jest 40 passed, chat-core 194. |
| 1.15.8 | 2026-09-18 | Claude | **Users can change their own password (FR-AUTH-004)**: the endpoint, the web page and the mobile screen all already existed, but `/change-password` was reachable **only** through the forced flow — `AppShell.tsx`'s `must_change_password` redirect and `LoginPage`/`app/index.tsx`/`app/login.tsx` — so a user could never change their password voluntarily. Adds an entry point on both clients: a lock button beside Sign out in the web rail, and a Settings row on mobile (`settings.changePassword`, en + th). Also fixes a real defect the voluntary path exposed: mobile cleared the token and sent the user to `/login` after a successful change, but `ChangePasswordAction` revokes every **other** session and deliberately keeps the current one (`whereKeyNot($currentSession->id)`), so that re-login was never required by the server — mobile now returns to `/rooms`, matching web, which already stayed signed in. |
| 1.15.7 | 2026-09-18 | Claude | **PWA + Web Push background notifications (FR-NOTI-003, API-074, DEC-077)**: the web client can now notify with the tab closed, which a page-level `Notification` can never do. Adds a web app manifest with maskable icons and Apple touch meta (making the site installable, the precondition for iOS push), `public/firebase-messaging-sw.js` handling `onBackgroundMessage` + `notificationclick` (focusing an existing tab rather than opening a duplicate), FCM Web token registration wired into the existing "Enable desktop notifications" gesture, and **API-074 focus reporting from web for the first time** — the server's rule that a push is silenced for the room a device is actively reading was previously dead for web clients. Ships **dormant**: with no `VITE_FIREBASE_*` build vars every entry point no-ops, and the Firebase SDK is a lazy import so it stays out of the initial bundle (main chunk +4 kB). `webPushStatus()` distinguishes `needs-install` from `unsupported` so iPhone users get the Add-to-Home-Screen instruction instead of a dead end. Cover: 11 tests in `packages/chat-core/src/web-push.test.ts` (config completeness, the iOS gate, SW URL construction with the VAPID key withheld, device-id persistence including hostile storage). |
| 1.15.6 | 2026-09-18 | Claude | **Native OS notification popups in the web client (FR-NOTI-003 foreground branch, FR-NOTI-007, TC-WEB-030)**: `apps/web` had no `Notification` API usage at all. A popup now renders from the existing `.notification.alert` handler in `EchoProvider` — rendered *inside* that handler rather than as a second `channel.listen` for the same event, because laravel-echo's `stopListening(event)` with no callback unbinds every listener for it. Permission is requested only from an explicit button in `NotificationCenter` (TC-WEB-030: never on page load), with a distinct message when the browser has blocked it. Suppression reuses the server's eligibility decision (mute / mention-only / DND / self / system all resolve before EVT-063 is broadcast) and adds a client rule: no popup for the room you are currently looking at, now applied to **every** alert kind rather than only `message`. The popup shows the room name but **no message text** — EVT-063 is deliberately content-free, and defaulting to a generic body gives a desktop the same guarantee `preview_in_push=false` gives a phone. Dedupe and body text live in `packages/chat-core` as `DesktopNotificationGate` / `desktopNotificationBody` per the repo's platform-agnostic rule, deliberately **not** reusing `NotificationGate`, whose 1-per-second throttle is correct for an audio chime and would silently swallow a burst of popups. **This is the foreground branch only**: a page-constructed `Notification` dies with the tab, so background delivery on a phone still requires the service worker + Web Push workstream. Cover: 6 tests in `packages/chat-core/src/notification.test.ts` (dedupe, eviction, no-rate-limit contrasted against the audio gate, kind→body mapping). |
| 1.15.5 | 2026-09-18 | Claude | **Push delivery repaired (FR-NOTI-002/003, DEC-076)**: the notification pipeline was fully built and tested but delivered nothing. `FcmPushSender` migrated from the decommissioned legacy FCM endpoint to **HTTP v1** (OAuth2 service-account assertion signed with the existing `firebase/php-jwt`, token cached ~1h; no new dependency), now emitting the §10 `android`/`apns` blocks and stringifying every `data` value as v1 requires; dead tokens detected via `error.details[].errorCode` (404) instead of the legacy in-body `results[].error`. Credentials renamed to the Appendix A names `FCM_PROJECT_ID` / `FCM_CREDENTIALS_JSON` and added to `apps/api/.env.example`, where no FCM variable had ever appeared — the reason a dead push path went unnoticed, since an unset key short-circuits to a no-op (now logged at warning, not info). **`preview_in_push` now reads the recipient, not the sender**: the setting is a recipient-side privacy control and was inverted, so it governed other people's lock screens and never the owner's; `TC-NOTI-014` had codified the inversion and is corrected, with a new regression test asserting a sender cannot suppress a recipient's preview. Verified against a pristine `main` clone on identical infrastructure: 547 → **548 passed, 0 failed** (3072 assertions), Pint clean. |
| 1.15.4 | 2026-09-18 | Claude | **MinIO images repointed to quay.io; README rewritten as a deployment guide**: upstream removed `minio/minio` and `minio/mc` from Docker Hub (`pull access denied ... repository does not exist`), so **both** `infra/docker-compose.yml` and `infra/docker-compose.prod.yml` now pull `quay.io/minio/{minio,mc}:latest` — before this a fresh clone could start neither the dev stack nor the prod stack. Same upstream images and tags, no behavioural change, so no DEC. README replaced: it still described a PH1 MVP with AI, push, search, mentions, edit/delete, typing and mobile listed as deferred (all shipped) and had no deployment section at all. The new guide documents the silent-failure modes — `infra/.env` (compose `${VAR}` substitution, wins inside containers) vs `apps/api/.env` (bind-mounted, read by Laravel) with the authoritative list of compose-referenced variables; `apps/api/.env` needing to exist and be `chown 82:82` before the first `up`; the wizard's `127.0.0.1` prefill being wrong inside a container; `AWS_ENDPOINT` host-root under SigV4; `LIVEKIT_URL` taking no path; the LiveKit UDP-buffer sysctl, dual-edit TURN port, `use_external_ip` on dynamic WANs and `renew-certificate.sh`'s hardcoded lineage; and the Cloudflare 521/Origin-Rule fix with its 100 MB body cap and edge-IP rate limiting. |
| 1.15.3 | 2026-09-17 | Claude | **AI provider `base_url` accepts http (FR-AI-019, DEC-075)**: `/admin/ai-providers/create` no longer requires `https://` — the Filament field validates the scheme is exactly `http` or `https` and warns that an http hop carries the API key in cleartext; `OpenAiCompatibleProvider::guardHost()` enforces the same pair in **every** environment, replacing a condition that only applied under `production` and therefore let any scheme through elsewhere. Same commit closes an SSRF bypass that http made reachable: an IP-literal host skipped the private-range check because `gethostbyname()` echoes it back into the "resolution failed" branch, so `http://10.0.0.1` / `http://169.254.169.254` passed the guard; literals are now detected and checked directly. The private-range gate and its `AI_ALLOW_PRIVATE_HOSTS` opt-in are otherwise unchanged. Cover: `tests/Feature/Ai/AiProviderSchemeTest.php` (http/https accepted, non-http schemes refused in production *and* local, DNS-resolved and literal private addresses refused, opt-in honoured, loopback still allowed). |
...
| OQ-010 | Branding/ชื่อจริงของแอป, icon, สี — **icon/logo ส่วนเดียวปิดแล้ว (DEC-082, FR-ADM-015)**; ชื่อจริงกับสียังเปิดอยู่ | PO/Design | ก่อน PH3 (store assets) |
