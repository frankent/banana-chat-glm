...
try {
  ...
  await check('TC-WEB-040-conversations-navigation', async () => {
    await navigate(sender, '/search');
    await sender.getByRole('button', {name:'Conversations', exact:true}).click();
    await expect(sender).toHaveURL(/\/$/);
    return {returnsToConversations:true};
  });
  ...
  await check('TC-CORE-023-scroll-search-read-viewport', async () => {
    ...
    await expect.poll(async () => (await client(recipient, 'rooms', fixture.slug)).find(r=>r.room.id===room.room.id).unread_count).toBe(0).catch(async e=>{console.log('READ_DIAGNOSTIC',await recipient.getByTestId('message-list').evaluate(el=>({focus:document.hasFocus(),visibility:document.visibilityState,top:el.scrollTop,height:el.scrollHeight,client:el.clientHeight,last:el.lastElementChild.getBoundingClientRect().toJSON()})));throw e;});
  ...
  await check('TC-MSG-060-reply-markdown-pin-typing', async () => {
    await navigate(sender, `/rooms/${dmId}`);
    await navigate(recipient, `/rooms/${dmId}`);
    await sender.getByTestId('composer-input').fill('Typing verification');
    await expect(recipient.getByTestId('typing-indicator')).toContainText('Review sender');
    await send('**Markdown original**');
    const original = recipient.getByTestId('message').filter({hasText:'Markdown original'}).first();
    await expect(original.locator('strong').filter({hasText:'Markdown original'})).toBeVisible();
    await original.hover();
    await original.getByRole('button',{name:'Reply',exact:true}).click();
    await expect(recipient.getByRole('button',{name:'Cancel reply'})).toBeVisible();
    await recipient.getByTestId('composer-input').fill('Reply from recipient');
    await recipient.getByTestId('send-button').click();
    await expect(sender.getByTestId('message').filter({hasText:'Reply from recipient'}).locator('.bc-reply-quote')).toContainText('Markdown original');
    await original.hover();
    await original.getByRole('button',{name:'Pin message',exact:true}).click();
    const pins = recipient.getByLabel('Pinned messages');
    await expect(pins).toContainText('Markdown original');
    await pins.getByRole('button').first().click();
    await expect(recipient).toHaveURL(/around_seq=/);
    await expect(original).toBeVisible();
    await pins.getByRole('button',{name:'Unpin message'}).click();
    await expect(pins).toHaveCount(0);
    await navigate(recipient, `/rooms/${dmId}`);
    return {typingNamed:true, markdown:true, replyQuoted:true, pinJump:true};
  });
  await check('TC-NOTE-001-browser-room-notebook', async () => {
    await sender.getByRole('button',{name:'Room notes',exact:true}).click();
    await sender.getByLabel('Note text').fill('# Shared note\n[Project link](https://example.com)');
    await sender.getByRole('button',{name:'Create note',exact:true}).click();
    await expect(sender.getByTestId('room-note')).toContainText('Shared note');
    await recipient.getByRole('button',{name:'Room notes',exact:true}).click();
    await expect(recipient.getByTestId('room-note')).toContainText('Shared note');
    await expect(recipient.getByTestId('room-note').getByRole('link',{name:'Project link'})).toHaveAttribute('href','https://example.com');
    await sender.getByRole('button',{name:'Edit note',exact:true}).click();
    await sender.getByLabel('Note text').fill('Updated note');
    await sender.getByRole('button',{name:'Save changes',exact:true}).click();
    await expect(recipient.getByTestId('room-note')).toContainText('Updated note');
    await recipient.screenshot({path:new URL('notes-desktop.png', import.meta.url).pathname});
    await sender.getByRole('button',{name:'Delete note',exact:true}).click();
    await expect(recipient.getByTestId('room-note')).toHaveCount(0);
    await sender.getByRole('button',{name:'Close notes'}).click();
    await recipient.getByRole('button',{name:'Close notes'}).click();
    return {create:true, realtime:true, edit:true, delete:true};
  });
  await check('TC-MEDIA-020-viewers-and-media-replies', async () => {
    // Ready file fixtures isolate viewer behavior from media-processing workers.
    const video = await sender.evaluate(async () => {
      const canvas = document.createElement('canvas'); canvas.width=160;canvas.height=90;
      const context=canvas.getContext('2d'); context.fillStyle='#72845f';context.fillRect(0,0,160,90);
      const stream=canvas.captureStream(5);const recorder=new MediaRecorder(stream,{mimeType:'video/webm'});const chunks=[];
      const done=new Promise(resolve=>{recorder.ondataavailable=e=>chunks.push(e.data);recorder.onstop=()=>resolve(new Blob(chunks,{type:'video/webm'}));});
      recorder.start();await new Promise(r=>setTimeout(r,1100));recorder.stop();const blob=await done;stream.getTracks().forEach(t=>t.stop());
      return btoa(String.fromCharCode(...new Uint8Array(await blob.arrayBuffer())));
    });
    const ids = JSON.parse(php(`
      $fixtures = [['image','image/png','fixture.png','iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+aStsAAAAASUVORK5CYII='], ['file','text/markdown','fixture.md',base64_encode('# Preview heading')], ['video','video/webm','fixture.webm','${video}']];
      $ids=[]; foreach($fixtures as [$kind,$mime,$name,$bytes]) {
        $id=(string)Illuminate\\Support\\Str::ulid();$key='ws/${fixture.workspace}/att/'.$id.'/original';
        Illuminate\\Support\\Facades\\Storage::disk('local')->put($key,base64_decode($bytes));
        App\\Models\\Attachment::forceCreate(['id'=>$id,'workspace_id'=>'${fixture.workspace}','uploader_id'=>'${fixture.senderId}','kind'=>$kind,'status'=>'ready','original_name'=>$name,'mime_type'=>$mime,'size_bytes'=>strlen(base64_decode($bytes)),'storage_key'=>$key]);$ids[]=$id;
      } echo json_encode($ids);
    `));
    const source = (await client(sender, 'messages', dmId, fixture.slug)).messages.at(-1);
    await client(sender, 'sendMessage', dmId, fixture.slug, 'Viewer fixture reply', crypto.randomUUID(), source.id, ids);
    const message = recipient.getByTestId('message').filter({hasText:'Viewer fixture reply'});
    await expect(message.locator('.bc-reply-quote')).toBeVisible();
    await message.getByRole('button',{name:'View fixture.png',exact:true}).click();
    await expect(recipient.getByRole('dialog')).toBeVisible();
    await expect.poll(()=>recipient.getByRole('dialog').locator('img').evaluate(img=>img.naturalWidth)).toBe(1);
    await recipient.getByRole('button',{name:'Close viewer'}).click();
    await message.getByTestId('attachment-file').click();
    await expect(recipient.getByRole('dialog')).toContainText('Preview heading');
    await recipient.keyboard.press('Escape');
    await message.getByRole('button',{name:'Open video viewer'}).click();
    await expect.poll(()=>recipient.getByRole('dialog').locator('video').evaluate(v=>v.readyState)).toBeGreaterThanOrEqual(2);
    await recipient.getByRole('button',{name:'Close viewer'}).click();
    return {imageLoaded:true, videoLoaded:true, markdownFilePreview:true, mediaReply:true};
  });
  await check('TC-WEB-041-delayed-media-layout', async () => {
    await navigate(recipient, '/search');
    const ids=JSON.parse(php(`$ids=[];for($i=0;$i<8;$i++){ $id=(string)Illuminate\\Support\\Str::ulid();
      $a=App\\Models\\Attachment::forceCreate(['id'=>$id,'workspace_id'=>'${fixture.workspace}','uploader_id'=>'${fixture.senderId}','kind'=>'image','status'=>'ready','original_name'=>'delayed-'.$i.'.png','mime_type'=>'image/png','size_bytes'=>68,'storage_key'=>'ws/${fixture.workspace}/att/'.$id.'/original','width'=>320,'height'=>180]);
      Illuminate\\Support\\Facades\\Storage::disk('local')->put($a->storage_key,base64_decode('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+aStsAAAAASUVORK5CYII='));$ids[]=$a->id;
    }echo json_encode($ids);`));
    await client(sender,'sendMessage',dmId,fixture.slug,'Delayed media layout verification',crypto.randomUUID(),undefined,ids);
    const pattern=/\/attachments\/[^/]+\/file\/original/;
    await recipient.route(pattern,async route=>{await new Promise(resolve=>setTimeout(resolve,1200));await route.fallback();});
    await navigate(recipient,`/rooms/${dmId}`);
    const images=recipient.getByTestId('message').filter({hasText:'Delayed media layout verification'}).getByTestId('attachment-image');
    await expect(images).toHaveCount(8);
    await expect.poll(()=>images.evaluateAll(xs=>xs.every(x=>x.complete&&x.naturalWidth>0))).toBe(true);
    await expect.poll(async()=> (await client(recipient,'rooms',fixture.slug)).find(r=>r.room.id===dmId).unread_count).toBe(0);
    const remaining=await recipient.getByTestId('message-list').evaluate(el=>el.scrollHeight-el.scrollTop-el.clientHeight);
    expect(remaining).toBeLessThan(5);
    await recipient.unroute(pattern);
    return {images:8,delayedDecode:true,unread:0,bottomGap:remaining};
  });
  await check('TC-WS-005-workspace-directory', async () => {
    await sender.getByRole('button',{name:'Workspace members',exact:true}).click();
    await sender.getByLabel('Search workspace members').fill(fixture.receiver);
    await sender.getByRole('button',{name:/Review receiver/}).click();
    await expect(sender).toHaveURL(new RegExp(`/rooms/${dmId}$`));
    return {directoryStartsDm:true};
  });
