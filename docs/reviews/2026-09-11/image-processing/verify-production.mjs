...
try {
 ...
 for (const format of ['png','jpeg','gif','webp','indexed.png']) await check('TC-MEDIA-021-real-upload-'+format,async()=>{
  ...
  await expect(chip).toHaveAttribute('data-status','ready',{timeout:45000});
  const delivered=sender.page.waitForResponse(r=>r.url().endsWith(`/rooms/${dm}/messages`)&&r.request().method()==='POST');
  const body=run+' Image pipeline '+format;
  await sender.page.getByTestId('composer-input').fill(body);await sender.page.getByTestId('send-button').click();
  const payload=await (await delivered).json();const attachment=payload.data.message.attachments[0];
  expect(attachment.status).toBe('ready');expect(attachment.width).toBe(320);expect(attachment.height).toBe(180);expect(attachment.urls.thumb_md).toBeTruthy();
  const item=recipient.page.getByTestId('message').filter({hasText:body});
  await expect(item).toBeVisible();
  await expect.poll(()=>item.getByTestId('attachment-image').evaluate(img=>img.naturalWidth),{timeout:15000}).toBeGreaterThan(0);
  await item.getByRole('button',{name:'View banana-qa-image.'+format,exact:true}).click();
  await expect.poll(()=>recipient.page.getByRole('dialog').locator('img').evaluate(img=>img.naturalWidth),{timeout:15000}).toBe(320);
  await recipient.page.getByRole('button',{name:'Close viewer'}).click();
 });
