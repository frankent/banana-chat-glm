...
try {
  ...
  await pages[0].screenshot({
    path: new URL("production-group-video.png", import.meta.url).pathname,
  });
  results.push({
  ...
  await pages[0].screenshot({
    path: new URL("production-mobile-video.png", import.meta.url).pathname,
  });
  ...
  if (pages[0])
    await pages[0].screenshot({
      path: new URL("production-failure.png", import.meta.url).pathname,
    });
  process.exitCode = 1;
} finally {
  await writeFile(
    new URL("production-results.json", import.meta.url),
