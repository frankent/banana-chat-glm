...
const fixture = JSON.parse(
  php(
    `$w=App\\Models\\Workspace::factory()->create(['slug'=>'${prefix}']);$users=[];for($i=0;$i<3;$i++){$u=App\\Models\\User::factory()->create(['username'=>'${prefix}'.$i,'display_name'=>'Call tester '.($i+1)]);$w->members()->attach($u->id,['role'=>'member']);$users[]=$u;} $rooms=[];foreach(['dm','group'] as $type){$r=App\\Models\\Room::create(['workspace_id'=>$w->id,'type'=>$type,'name'=>$type==='group'?'Design meeting':null,'created_by'=>$users[0]->id,'member_count'=>$type==='dm'?2:3]);foreach(array_slice($users,0,$type==='dm'?2:3) as $u)$r->members()->attach($u->id,['workspace_id'=>$w->id,'role'=>'member']);$rooms[$type]=$r->id;}echo json_encode(['users'=>array_map(fn($u)=>$u->username,$users),'slug'=>$w->slug,'rooms'=>$rooms,'workspace'=>$w->id]);`,
  ),
);
...
try {
    ...
    await ctx.route(/\/(api|broadcasting)\//, async (route) => {
      ...
      try {
        ...
        if (
          ...
          /\/calls\/[^/]+\/join$/.test(target.pathname) &&
          res.ok()
        )
  ...
  await pages[0].screenshot({
    path: new URL("group-video.png", import.meta.url).pathname,
  });
  results.push({
  ...
  await pages[0].screenshot({
    path: new URL("mobile-video.png", import.meta.url).pathname,
  });
  ...
  for (const credential of credentials) {
    const gate = await fetch("http://localhost:18880/rtc", {
