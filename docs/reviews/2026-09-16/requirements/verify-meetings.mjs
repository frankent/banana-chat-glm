...
async function context() {
  ...
  await ctx.addInitScript(() => {
    ...
    window.__playbackGains = [];
    const NativeAudioContext = window.AudioContext;
    window.AudioContext = class extends NativeAudioContext {
      createGain() { const node = super.createGain(); window.__playbackGains.push(node); return node; }
    };
...
try {
  ...
  for (const page of [host, guest, member]) {
    ...
    expect(await page.evaluate(() => window.__captureTracks.filter(t => t.kind === 'video' && t.readyState === 'live').length)).toBe(0);
    await expect(page.locator('button[data-lk-source="camera"]')).toHaveAttribute('data-lk-enabled', 'false');
  }
  results.push({test:'TC-CALL-032 member and guest cameras are off on entry', status:'passed'});
  for (const page of [host, guest, member]) {
    await page.locator('button[data-lk-source="camera"]').click();
  }
  for (const page of [host, guest, member]) {
    await media(page);
    await expect(page.locator(".bc-call-count")).toHaveText("3 participants");
  ...
  await expect
    ...
    .toBe(true);
  await expect.poll(() => guest.evaluate(() => window.__playbackGains.some(g => Math.abs(g.gain.value - 1.5) < .01))).toBe(true);
  await guest.getByRole('slider', {name:'Speaker volume'}).fill('200');
  await expect.poll(() => guest.evaluate(() => window.__playbackGains.some(g => Math.abs(g.gain.value - 2) < .01))).toBe(true);
  results.push({test:'TC-CALL-031 remote playback gain defaults to 150% and adjusts to 200%',status:'passed'});
  await host.getByRole('button', {name:/Share screen/}).click();
  await expect(guest.locator('.bc-call-focus')).toHaveAttribute('data-focus-source','screen_share');
  await member.getByRole('button', {name:/Share screen/}).click();
  await expect(guest.getByLabel('Focus participant or screen').locator('option').filter({hasText:'— screen'})).toHaveCount(2);
  await guest.getByLabel('Focus participant or screen').selectOption({label:'Jordan Member — screen'});
  await guest.screenshot({path:new URL((prod ? 'production-' : '') + 'screen-focus.png', import.meta.url).pathname});
  await expect(guest.locator('.bc-call-focus')).toContainText('Jordan Member');
  await guest.getByLabel('Focus participant or screen').selectOption({label:'Alex Partner (Guest) — participant'});
  await expect(guest.locator('.bc-call-focus')).toHaveAttribute('data-focus-source','camera');
  await guest.getByRole('button', {name:'Full screen', exact:true}).click();
  await expect.poll(() => guest.evaluate(() => !!document.fullscreenElement)).toBe(true);
  await guest.evaluate(() => document.exitFullscreen());
  await guest.getByRole('button', {name:'Automatic view'}).click();
  await expect(guest.locator('.bc-call-focus')).toHaveAttribute('data-focus-source','screen_share');
  await host.getByRole('button', {name:/Stop screen share/}).click();
  await expect(guest.locator('.bc-call-focus')).toContainText('Jordan Member');
  await member.getByRole('button', {name:/Stop screen share/}).click();
  await expect(guest.locator('.bc-call-focus')).toHaveCount(0);
  results.push({test:'TC-CALL-030 concurrent shares auto-focus, manual guest focus, fullscreen and fallback',status:'passed'});
  ...
  await host.getByRole("button", { name: "Stop screen share", exact: true }).click();
  results.push({
  ...
  await expect
    ...
    .toBe(true);
  await expect.poll(() => guest.evaluate(() => window.__playbackGains.every(g => g.context.state === 'closed'))).toBe(true);
