...
try {
  for (const username of fixture.users) {
    ...
    if (!prod) await ctx.route(/\/(api|broadcasting)\//, async route => {
      const target = new URL(route.request().url()); target.host = "localhost:18000";
      try { const response = await route.fetch({url:target.toString()}); await route.fulfill({response}); }
      catch(e) { if(!/closed|disposed/.test(e.message)) throw e; }
    });
  ...
  results.push({test:'TC-CALL-032 group video cameras opt in',status:'passed'});
  for (const page of pages) {
    await page.bringToFront();
  ...
  await pages[0]
    .locator(".bc-call-stage")
    .getByRole("button", { name: "Stop screen share", exact: true })
    .click();
  results.push({
  ...
  await pages[1]
    ...
    .getByRole("button", { name: "Join", exact: true })
    .click();
  expect(await pages[1].evaluate(() => window.__captureTracks.filter(t => t.kind === 'video' && t.readyState === 'live').length)).toBe(0);
  await pages[1].locator('button[data-lk-source="camera"]').click();
  await media(pages[0], "video");
  ...
  for (const credential of credentials) {
    const gate = await fetch((prod ? base : "http://localhost:18880") + "/rtc", {
  ...
  await writeFile(
    new URL((prod ? "production-" : "") + "private-calls-results.json", import.meta.url),
