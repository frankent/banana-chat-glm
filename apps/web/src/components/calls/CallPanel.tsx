...
export default function CallPanel({
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        try {
          ...
          if (active.call.kind === "video" && !cancelled)
    ...
    const timer = setInterval(() => {
      void endpoints
        .calls(active.slug)
        .then((result) => {
          if (!cancelled && !result.calls.some((c) => c.id === active.call.id))
            leaveCall();
        })
        .catch(() => {});
    }, 5000);
  ...
  }, [active.call.id, active.call.kind, active.slug, active.token, active.url]);
  if (!room) return null;
  return (
    <div
      className={`bc-call-stage ${minimized ? "is-minimized" : ""} ${active.call.kind === "voice" ? "is-voice" : ""}`}
      data-lk-theme="default"
      role="dialog"
      aria-label={active.call.kind === "voice" ? "Voice call" : "Video call"}
    >
      <header>
        <div>
          <span className="bc-call-live" />
          <strong>{active.call.room_name || active.call.caller_name}</strong>
          <small>
            {connecting
              ? "Connecting…"
              : active.call.kind === "voice"
                ? "Voice call"
                : "Video meeting"}
          </small>
        </div>
      ...
      <RoomContext.Provider value={room}>
        ...
        <footer>
          <ControlBar
            ...
            controls={{
              microphone: true,
              camera: active.call.kind === "video",
              screenShare: active.call.kind === "video",
              chat: false,
              leave: false,
            }}
          />
